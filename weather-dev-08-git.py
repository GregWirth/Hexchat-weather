...
HOST = "irc.server.name"
PORT = 6667
USER = "user-nick"
CHANNELS = ["cannel01", "#channel02"]
TRIGGER = "#zz"
RATE_LIMIT = 7
RATE_LIMIT_TIME = 60
IGNORE_TIME = 180
...
class IrcBot:
    ...
    async def handle_privmsg(self, line):
        prefix, command, channel, *params = line.split()
        message = ' '.join(params)[1:]
        user = prefix.split('!')[0][1:]

        if message.startswith(TRIGGER):
            await self.handle_weather_command(user, channel, message[len(TRIGGER):].strip())
