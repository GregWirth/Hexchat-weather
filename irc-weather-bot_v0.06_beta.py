...
import re
from contextlib import suppress  # Import for suppressing exceptions
...
class IrcBot:
    ...
    def __init__(self):
        ...
        self.authenticated = asyncio.Event()  # Event to signal when authentication is complete
    ...
    async def connect(self):
        ...
        try:
            ...
            # Authenticate before joining channels
            await self.authenticate()
            auth_success = await self.wait_for_nickserv_response("You are now identified")
            if not auth_success:
        ...
        async with self.reconnect_lock:
            ...
            while self.running:
                try:
                    ...
                    await self.connect()
                    # Authentication and joining channels are handled within connect()
    ...
    def format_weather_data(self, data):
        ...
        try:
            ...
            # Extract all the required fields with default values
            ...
            mintemp_f = forecast.get('mintemp_f', 'N/A')
            mintemp_c = forecast.get('mintemp_c', 'N/A')
            maxtemp_f = forecast.get('maxtemp_f', 'N/A')
            maxtemp_c = forecast.get('maxtemp_c', 'N/A')
            daily_chance_of_rain = forecast.get('daily_chance_of_rain', 0)
            daily_chance_of_snow = forecast.get('daily_chance_of_snow', 0)
            totalsnow_cm = forecast.get('totalsnow_cm', 0.0)
            try:
    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                await self.start_background_tasks()  # Start background tasks
    ...
    async def handle_notice(self, prefix, params):
        ...
        if sender_nick.lower() == 'nickserv':
            if 'identify' in message.lower() and 'registered' in message.lower():
                ...
                self.authenticated.set()  # Signal that authentication is complete
    ...
    async def authenticate(self):
        ...
        await self.send_privmsg("NickServ", f"IDENTIFY {PASSWORD}")
