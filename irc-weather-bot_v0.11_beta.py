    ...
    async def handle_privmsg(self, prefix, params):
        ...
        try:
            ...
            if channel == self.current_nick:
                ...
                elif message_lower == STAB_TRIGGER.lower():
                    await self.handle_stab_command(channel)
        except Exception as e:
            logger.exception(f"Exception in handle_privmsg: {e}")

    async def handle_stab_command(self, channel):
        """Respond to the stab trigger."""
        response_line = self.stab_responder.get_random_response()
        message = f"hftb stabs plazma {response_line}"
        await self.send_message(channel, message)
        logger.info(f"Sent stab response to {channel}.")
