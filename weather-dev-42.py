...
class IrcBot:
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.error(f"Connection refused by IRC server at {HOST}:{PORT}")
            raise
        ...
        while True:
            try:
                ...
                if command == '001':
                    ...
                    logger.debug(f"Unhandled message during registration: {line}")
    ...
    async def join_channel(self, channel):
        ...
        if self.writer is None:
            logger.error("Cannot join channel, no active connection.")
            return
        async with self.writer_lock:
            self.writer.write(f"JOIN {channel}\r\n".encode())
            await self.writer.drain()
    ...
    async def ping_chanserv(self):
        ...
        while self.running:
            ...
            try:
                ...
                try:
                    self.ping_event.clear()
                    await asyncio.wait_for(self.ping_event.wait(), timeout=120)
    ...
    async def process_line(self, line):
        ...
        if command == 'PING':
            ...
            if "closing link" in error_message.lower():
                ...
                await self.reconnect(is_netsplit=True)
            else:
                await self.reconnect()
        elif command == 'KICK':
    ...
    async def wait_for_nickserv_response(self, expected_message, timeout=30):
        """Wait for a specific message from NickServ."""
            ...
            try:
                ...
                if command == 'NOTICE':
                    ...
                    if sender_nick.lower() == 'nickserv' and expected_message.lower() in message.lower():
                        logger.debug(f"Received expected message from NickServ: {message}")
                        return True
            except asyncio.TimeoutError:
                continue
            except Exception as e:
                ...
                return False
        logger.warning(f"Did not receive expected message from NickServ within timeout.")
        return False
    ...
    async def reconnect(self, is_netsplit=False):
        ...
        retry_delay = 10 if not is_netsplit else 5  # Shorter delay for netsplits
        retries = 0
